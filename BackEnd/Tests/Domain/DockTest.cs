...
public class DockTest
{
    ...
    public void WhenPassingInvalidParameters_ThenThrowsException(string dockName, string dockLocation, double length, double depth, double draft, bool useVesselTypes)
    {
        ...
        if (!useVesselTypes)
        {
            ...
            Assert.Throws<ArgumentException>(() => validDock.UpdateVesselTypes(vesselTypesParam));
            Assert.Throws<ArgumentException>(() => validDock.UpdateVesselTypes(new HashSet<VesselType> { }));
        }
    }
}