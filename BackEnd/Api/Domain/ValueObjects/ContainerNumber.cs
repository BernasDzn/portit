...
namespace Api.Domain.ValueObjects
{
    ...
    public class ContainerNumber
    {
        ...
        public string CheckDigit { get; private set; } = string.Empty;

        public string FullCode => OwnerCode + SerialNumber + CheckDigit;

        // EF Core needs this
        protected ContainerNumber() { }

        public ContainerNumber(string code)
        {
            if (!IsValidContainerNumber(code))
                throw new ArgumentException("Invalid container number format.", nameof(code));

            Id = Guid.NewGuid();

            OwnerCode = code.Substring(0, 4).ToUpperInvariant();
            SerialNumber = code.Substring(4, 6);
            CheckDigit = code.Substring(10, 1);
        }

        private static bool IsValidContainerNumber(string code)
        {
        ...
        private static string CalculateCheckDigit(string first10)
        {
