...
public class VesselVisitNotificationController : ControllerBase, IVesselVisitNotificationController
{
    ...
    [HttpGet("getAllIds", Name = "GetAllIds")]
    public async Task<ActionResult<IEnumerable<string>>> GetAllIds()
    {
        try
        {
            IEnumerable<string> ids = await _notificationService.GetVesselVisitNotificationIds();
            return Ok(ids);
        }
        catch (System.Exception e)
        {
    ...
    [HttpGet("getAllAcceptedVVNs", Name = "GetAllAcceptedVVNs")]
    public async Task<ActionResult<IEnumerable<string>>> GetAllAcceptedVVNs()
    {
        try
        {
            IEnumerable<string> ids = await _notificationService.GetAllAcceptedVVNIds();
            return Ok(ids);
        }
        catch (System.Exception e)
        {
            _logger.LogCritical("Error retrieving accepted vessel visit notifications, {Message}", e.Message);
            return StatusCode(500, "An error occurred while retrieving accepted vessel visit notifications.");
        }
    }
    ...
    [HttpGet("onMonth", Name = "GetVesselVisitNotificationsOnMonth", Order = 0)]
    [Authorize(Policy = "VesselVisitNotification.View")]
    ...
    [HttpGet("{id}", Name = "GetVesselVisitNotificationById", Order = 1)]
