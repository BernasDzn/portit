...
|        Incident             |     Entity     | An unplanned event or disruption occurring during port operations, with details on type, severity, timing, and affected vessel visits.                                    |
|       Incident Type         |     Entity     | A category of incident with name, description, severity level, and support for hierarchical incident type relationships through subtypes.                                |
|      Incident Severity      |     Value      | The level of impact of an incident, classified as Minor, Major, or Critical.                                                                                             |
|      Operation Plan         |     Entity     | A scheduled plan for executing operations during a vessel visit, detailing the sequence of operations, assigned resources, and execution metadata.                        |
|        Operation            |     Value      | A single operational task within an operation plan, including type, start/end times, assigned resources, and optional payload information.                              |
|    Operation Status         |     Value      | The execution state of an operation, classified as Pending, Started, Delayed, or Completed.                                                                            |
|  Vessel Visit Execution     |     Entity     | The execution record of a vessel visit, tracking operations performed, their status, key dates (open/close), dock assignment, and berth information.                      |
|  Vessel Visit Execution Status |   Value      | The overall execution state of a vessel visit, classified as Open or Closed.                                                                                            |
|      Task Category          |     Entity     | A classification system for operational tasks, with unique identifier, name, and description defining the type of work to be performed.                                 |
| Operation Plan Metadata     |     Value      | Metadata associated with an operation plan, recording who created it, when it was created, and which algorithm was used to generate it.                               |
